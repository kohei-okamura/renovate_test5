...
/**
 * LtcsContract update のテスト
 * PUT /users/{userId}/ltcs-contracts/{id}
 */
final class UpdateLtcsContractCest extends ContractTest
{
    ...
    public function succeedAPICall(ApiTester $I): void
    {
        ...
        $params = $this->createParams($contract);

        $I->sendPUT(
            "users/{$contract->userId}/ltcs-contracts/{$contract->id}",
            $params
        );
    ...
    public function succeedAPICallWithStatusIsFormal(ApiTester $I): void
    {
        ...
        $staff = $this->examples->staffs[0];
        $I->actingAs($staff);
        $contract = $this->examples->contracts[29]->copy([
            'status' => ContractStatus::formal(),
            'contractedOn' => Carbon::create(2019, 1, 1),
            'terminatedOn' => Carbon::create(2019, 12, 31),
            'ltcsPeriod' => ContractPeriod::create([
                'start' => Carbon::create(2019, 5, 17),
                'end' => Carbon::create(2019, 12, 15),
            ]),
        ]);
        ...
        $I->sendGET("users/{$contract->userId}/ltcs-contracts/{$contract->id}");
        $I->seeResponseCodeIs(HttpCode::OK);
    ...
    {
        $I->wantTo('failed with 404 Not Found when Invalid ID');
        ...
        $I->sendPUT(
            "users/{$contract->userId}/ltcs-contracts/{$id}",
            $this->createParams($contract)
        );
    ...
    {
        $I->wantTo('failed with 400 Bad Request when officeId is outside organization.');
    ...
    {
        $I->wantTo('failed with 400 Bad Request when OfficeId is not accessible');
    ...
    {
        $I->wantTo('failed with 404 Not Found when Invalid UserID');
        ...
        $I->sendPUT(
            "users/{$userId}/ltcs-contracts/{$contract->id}",
            $this->createParams($contract)
        );
    ...
    {
        $I->wantTo('failed with 404 Not Found when User is outside Organization');
        ...
        $I->sendPUT(
            "users/{$user->id}/ltcs-contracts/{$contract->id}",
            $this->createParams($contract)
        );
    ...
    {
        $I->wantTo('failed with 404 Not Found when ID is outside Organization');

        $staff = $this->examples->staffs[30];
        $I->actingAs($staff);

        $contract = $this->examples->contracts[35];
    ...
    {
        $I->wantTo('failed with 404 Not Found when UserID is not in accessible Office');
        ...
        $I->sendPUT(
            "users/{$target->userId}/ltcs-contracts/{$target->id}",
            $this->createParams($contract)
        );
    ...
    private function getContract(array $attrs = []): Contract
    {
        ...
        return $this->examples->contracts[1]->copy($attrs + $values);
    }
