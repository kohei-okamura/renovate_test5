...
/**
 * UserBilling createInvoice のテスト.
 * POST /user-billing-invoices
 */
class CreateUserBillingInvoicesCest extends UserBillingTest
{
    ...
    public function suceedAPICall(ApiTester $I)
    {
        ...
        $I->sendPost('/user-billing-invoices', compact('ids', 'issuedOn'));
        ...
        $I->seeLogMessage(1, LogLevel::INFO, '利用者請求：請求書生成ジョブ終了', [
            'organizationId' => $staff->organizationId,
            'staffId' => $staff->id,
        ]);
    ...
    public function failWithBadRequestWhenIdsIsNoting(ApiTester $I)
    {
        ...
        $I->sendPost('/user-billing-invoices', compact('issuedOn'));
    ...
    /**
     * IDが配列ではない場合に400が返るテスト.
     *
     * @param \ApiTester $I
     */
    public function failWithBadRequestWhenIdsIsNotArray(ApiTester $I)
    {
        $I->wantTo('fail with Bad Request when IDs is not array');

        $staff = $this->examples->staffs[0];
        $I->actingAs($staff);

        $ids = $this->createIds($staff)[0];
        $issuedOn = '2021-11-10T00:00:00Z';

        $I->sendPost('/user-billing-invoices', compact('ids', 'issuedOn'));

        $I->seeResponseCodeIs(HttpCode::BAD_REQUEST);
        $I->seeResponseContainsJson(['errors' => ['ids' => ['配列にしてください。']]]);
        $I->seeLogCount(0);
    }

    /**
     * 指定した ID の利用者請求が存在しない場合に400が返るテスト.
     *
     * @param \ApiTester $I
     */
    public function failWithBadRequestWhenUserBillingDoesNotExist(ApiTester $I)
    {
        $I->wantTo('fail with Bad Request when user billing does not exist');
    ...
    public function failWithBadRequestWhenIssuedOnIsNoting(ApiTester $I)
    {
        ...
        $I->sendPost('/user-billing-invoices', compact('ids'));
     ...
     * @return array
     */
    private function createIds(Staff $staff): array
    {
