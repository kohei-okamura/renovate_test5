...
/**
 * 障害福祉 重訪 辞書インポートMixin.
 */
trait DwsVisitingCareForPwsdDictionaryImportMixin
{
     ...
     * @param \ApiTester $I
     */
    public function _beforeMixinDwsVisitingCareForPwsdDictionaryImport(ApiTester $I)
    {
        // 辞書定義
        $dictionaries = [
            ['filename' => 'dict-dws-12.csv', 'effectivatedOn' => '2021-02-01', 'note' => '令和1年10月版'],
            ['filename' => 'dict-dws-12_R304.csv', 'effectivatedOn' => '2021-04-01', 'note' => '令和3年4月版'],
        ];
    ...
    private static function import(ApiTester $I, array $dictionaries): void
    {
        if (!self::$isImport) {
            ...
            foreach ($dictionaries as $index => $dictionary) {
                ...
                $id = $index + 10;
                $ret = system("php {$artisan} {$env} dws-visiting-care-for-pwsd-dictionary:import {$id} {$filename} {$dictionary['effectivatedOn']} {$note}");
