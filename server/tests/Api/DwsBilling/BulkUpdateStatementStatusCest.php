...
/**
 * Bulk Statement Status update のテスト(Dws).
 * POST /dws-billings/{billingId}/statement-status-update
 */
class BulkUpdateStatementStatusCest extends Test
{
    ...
    public function succeedAPICall(ApiTester $I)
    {
        ...
        $I->actingAs($staff);
        $billingId = $this->examples->dwsBillingStatements[21]->dwsBillingId;
        $ids = [
            $this->examples->dwsBillingStatements[21]->id,
            $this->examples->dwsBillingStatements[22]->id,
        ];

        $I->sendPost(
            "/dws-billings/{$billingId}/statement-status-update",
            $this->params($ids)
        );
        ...
        $I->seeLogMessage(1, LogLevel::INFO, '障害福祉サービス：明細書が更新されました', [
            'id' => '',
            'organizationId' => $staff->organizationId,
            'staffId' => $staff->id,
        ]);
