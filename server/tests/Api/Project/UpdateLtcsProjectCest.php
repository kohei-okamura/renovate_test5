...
/**
 * LtcsProject update のテスト.
 * PUT /users/{userId}/ltcs-projects{id}
 */
class UpdateLtcsProjectCest extends LtcsProjectTest
{
    ...
    public function succeedAPICall(ApiTester $I)
    {
        ...
        $I->sendPUT("users/{$ltcsProject->userId}/ltcs-projects/{$ltcsProject->id}", $this->defaultParam($ltcsProject));

        $I->seeResponseCodeIs(HttpCode::OK);
        $I->seeLogCount(1);
        $I->seeLogMessage(0, LogLevel::INFO, '介護保険サービス：計画が更新されました', [
            'id' => $ltcsProject->id,
    ...
    public function succeedAPICallWhenOwnExpenseProgramIsSet(ApiTester $I)
    {
        ...
        $I->sendPut("users/{$this->examples->ltcsProjects[0]->userId}/ltcs-projects/{$this->examples->ltcsProjects[0]->id}", $ltcsProject);

        $I->seeResponseCodeIs(HttpCode::OK);
        $I->seeLogCount(1);
        $I->seeLogMessage(0, LogLevel::INFO, '介護保険サービス：計画が更新されました', [
            'id' => $this->examples->ltcsProjects[0]->id,
            'organizationId' => $staff->organizationId,
            'staffId' => $staff->id,
        ]);
    }
    ...
    public function failWithBadRequestWhenServiceOptionIsInvalidForLtcsProject(ApiTester $I)
    {
        ...
        $staff = $this->examples->staffs[0];
        $I->actingAs($staff);
        $ltcsProject = $this->examples->ltcsProjects[0]->copy([
    ...
    public function failedWithNotFoundWhenIdIsString(ApiTester $I)
    {
        ...
        $I->actingAs($staff);
        $ltcsProject = $this->examples->ltcsProjects[0];
        $userId = $ltcsProject->userId;

        $I->sendPUT("users/{$userId}/ltcs-projects/id", $this->defaultParam($ltcsProject));
