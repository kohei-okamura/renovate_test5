...
/**
 * {@link \App\Http\Requests\UpdateLtcsBillingStatementStatusRequest} のテスト.
 */
final class UpdateLtcsBillingStatementStatusRequestTest extends Test
{
    use ConfigMixin;
    ...
    private UpdateLtcsBillingStatementStatusRequest $request;
    ...
    public static function _setUpSuite(): void
    {
        self::beforeEachSpec(function (self $self): void {
            $self->request = new UpdateLtcsBillingStatementStatusRequest();
            ...
            $self->lookupLtcsBillingStatementUseCase
                ->allows('handle')
                ->andReturn(Seq::from(
                    $self->examples->ltcsBillingStatements[0]->copy(
                        [
    ...
    public function describe_validation(): void
    {
        ...
        $examples = [
            ...
            'when status cannot be updated' => [
                ['status' => ['状態を更新できません。']],
                ['status' => LtcsBillingStatus::checking()->value()],
                ['status' => $input['status']],
            ],
        ];
        $this->should(
