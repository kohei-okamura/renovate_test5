...
/**
 * {@link \App\Http\Requests\CreateDwsBillingCopayCoordinationRequest} のテスト
 */
class CreateDwsBillingCopayCoordinationRequestTest extends Test
{
    use CarbonMixin;
    use ConfigMixin;
    ...
    protected CreateDwsBillingCopayCoordinationRequest $request;
    ...
    {
        static::beforeEachSpec(function (CreateDwsBillingCopayCoordinationRequestTest $self): void {
            ...
            $self->request = new CreateDwsBillingCopayCoordinationRequest();
            ...
            $self->dwsBillingStatementFinder
                ...
                ->andReturn(
                    FinderResult::from(
                        [
                            $self->examples->dwsBillingStatements[0]->copy(['copayLimit' => 50000]),
                        ],
                        Pagination::create()
                    )
                );
            ...
            $self->lookupOfficeUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::createBillings(), self::NOT_EXISTING_ID)
                ->andReturn(Seq::empty())
                ->byDefault();
            ...
            $self->getOfficeListUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), $self->examples->offices[2]->id)
                ->andReturn(Seq::from($self->examples->offices[1]->copy(['dwsGenericService' => null])))
                ->byDefault();
        });
    }
    ...
    public function describe_validation(): void
    {
        ...
        $examples = [
            ...
            'when items.*.officeId does not have dwsGenericService' => [
                ['items.0.officeId' => ['障害福祉サービスを提供していない事業所です。']],
                ['items.0.officeId' => $this->examples->offices[2]->id],
                ['items.0.officeId' => $this->dwsBillingCopayCoordination->items[0]->office->officeId],
            ],
            ...
            'when items.*.subtotal.copay is over copayLimit' => [
                ['items.0.subtotal.copay' => ['利用者負担上限月額以下で入力してください。']],
                ['items.0.subtotal.copay' => 99999],
                ['items.0.subtotal.copay' => $this->dwsBillingCopayCoordination->items[0]->subtotal->copay],
            ],
    ...
    private function defaultInput(): array
    {
        ...
        return [
            'dwsBundleId' => $dwsBillingCopayCoordination->dwsBillingBundleId,
