...
/**
 * {@link \App\Http\Requests\CreateDwsProjectRequest} のテスト.
 */
class CreateDwsProjectRequestTest extends Test
{
    use CarbonMixin;
    use ConfigMixin;
    ...
    protected CreateDwsProjectRequest $request;
    ...
    {
        static::beforeEachSpec(function (CreateDwsProjectRequestTest $self): void {
            $self->dwsProject = $self->examples->dwsProjects[0];
            $self->request = new CreateDwsProjectRequest();
            ...
            $self->lookupOfficeUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), [Permission::createDwsProjects()], self::NOT_EXISTING_ID)
            ...
            $self->lookupOwnExpenseProgramUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::createDwsProjects(), $self->examples->ownExpensePrograms[2]->id)
     ...
     */
    private function expectedPayload(array $input): DwsProject
    {
