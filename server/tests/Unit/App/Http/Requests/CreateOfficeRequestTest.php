...
/**
 * CreateOfficeRequest のテスト
 */
final class CreateOfficeRequestTest extends Test
{
    use CarbonMixin;
    use ConfigMixin;
    ...
    protected CreateOfficeRequest $request;
    ...
    public static function _setUpSuite(): void
    {
        self::beforeEachSpec(function (self $self): void {
            $self->request = new CreateOfficeRequest();
    ...
    {
        $this->should('return Office', function (): void {
     ...
     */
    private function expectedPayload(array $input): Office
    {
        $values = [
            'abbr' => $input['abbr'] ?? '',
            ...
            'addr' => new Addr(
                ...
                prefecture: $input['prefecture'] === null
                    ? Prefecture::from(0)
                    : Prefecture::from($input['prefecture']),
                city: $input['city'] ?? '',
                street: $input['street'] ?? '',
        ...
        if (!$this->isInternal($input)) {
            $inputDwsGenericService['dwsAreaGradeId'] = null;
        ...
        if (!$this->isInternal($input)) {
            $inputLtcsCareManagementService['ltcsAreaGradeId'] = null;
        ...
        if (!$this->isInternal($input)) {
            $inputLtcsHomeVisitLongTermCareService['ltcsAreaGradeId'] = null;
