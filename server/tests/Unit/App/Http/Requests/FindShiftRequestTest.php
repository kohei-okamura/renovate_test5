...
/**
 * FindShiftRequest のテスト.
 */
class FindShiftRequestTest extends Test
{
    use ConfigMixin;
    use ContextMixin;
    ...
    private FindShiftRequest $request;
    ...
    {
        static::beforeEachSpec(function (FindShiftRequestTest $self): void {
            ...
            $self->lookupUserUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::listShifts(), self::NOT_EXISTING_ID)
                ->andReturn(Seq::empty());
            ...
            $self->request = (new FindShiftRequest())->replace($self->input());
    ...
    public function describe_filterParams(): void
    {
        ...
        $this->should('set carbon params to null when specify empty', function (): void {
            $request = (new FindShiftRequest())->replace(['start' => '', 'end' => '']);
