...
/**
 * UpdateUserDwsSubsidyRequest のテスト
 */
class UpdateUserDwsSubsidyRequestTest extends Test
{
    use ConfigMixin;
    use ExamplesConsumer;
    use UnitSupport;

    protected UpdateUserDwsSubsidyRequest $request;
    ...
    {
        static::beforeEachSpec(function (UpdateUserDwsSubsidyRequestTest $self): void {
            $self->faker = app(Generator::class);
            $self->request = new UpdateUserDwsSubsidyRequest();
    ...
    public function describe_payload(): void
    {
        ...
        $this->should('return assoc with factor when subsidyType is benefitRate', function (): void {
        ...
        $this->should('return assoc with factor when subsidyType is copayRate', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
        ...
        $this->should('return assoc with factor none when subsidyType is neither benefitRate nor copayRate', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
        ...
        $this->should('return assoc with benefitRate when subsidyType is benefitRate', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
                $assoc['subsidyType'] = UserDwsSubsidyType::benefitRate()->value();
            });
            // リクエスト内容を反映させるために initialize() を利用する
        ...
        $this->should('return assoc with benefitRate 0 when subsidyType is not benefitRate', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
        ...
        $this->should('return assoc with copayRate when subsidyType is copayRate', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
                $assoc['subsidyType'] = UserDwsSubsidyType::copayRate()->value();
            });
            // リクエスト内容を反映させるために initialize() を利用する
        ...
        $this->should('return assoc with copayRate 0 when subsidyType is not copayRate', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
                $assoc['subsidyType'] = UserDwsSubsidyType::benefitAmount()->value();
            });
            // リクエスト内容を反映させるために initialize() を利用する
        ...
        $this->should('return assoc with benefitAmount when subsidyType is benefitAmount', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
                $assoc['subsidyType'] = UserDwsSubsidyType::benefitAmount()->value();
            });
            // リクエスト内容を反映させるために initialize() を利用する
        ...
        $this->should('return assoc with benefitAmount 0 when subsidyType is not benefitAmount', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
                $assoc['subsidyType'] = UserDwsSubsidyType::benefitRate()->value();
            });
            // リクエスト内容を反映させるために initialize() を利用する
        ...
        $this->should('return assoc with copayAmount when subsidyType is copayAmount', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
                $assoc['subsidyType'] = UserDwsSubsidyType::copayAmount()->value();
            });
            // リクエスト内容を反映させるために initialize() を利用する
        ...
        $this->should('return assoc with copayAmount 0 when subsidyType is not copayAmount', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
                $assoc['subsidyType'] = UserDwsSubsidyType::copayRate()->value();
            });
            // リクエスト内容を反映させるために initialize() を利用する
        ...
        $this->should('return assoc with rounding when subsidyType is benefitRate', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
                $assoc['subsidyType'] = UserDwsSubsidyType::benefitRate()->value();
            });
            // リクエスト内容を反映させるために initialize() を利用する
        ...
        $this->should('return assoc with rounding when subsidyType is copayRate', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
                $assoc['subsidyType'] = UserDwsSubsidyType::copayRate()->value();
            });
            // リクエスト内容を反映させるために initialize() を利用する
        ...
        $this->should('return assoc with rounding none when subsidyType is neither benefitRate nor copayRate', function (): void {
            $input = tap($this->defaultInput(), function (array &$assoc): void {
                $assoc['subsidyType'] = UserDwsSubsidyType::benefitAmount()->value();
            });
            // リクエスト内容を反映させるために initialize() を利用する
