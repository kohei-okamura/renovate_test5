...
/**
 * CreateShiftRequest のテスト
 */
class CreateShiftRequestTest extends Test
{
    use CarbonMixin;
    use ConfigMixin;
    use ContractExample;
    ...
    protected CreateShiftRequest $request;
    private Shift $shift;
    ...
    {
        static::beforeEachSpec(function (CreateShiftRequestTest $self): void {
            $self->request = new CreateShiftRequest();
            ...
            $self->lookupContractUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::createShifts(), $self->shift->userId, $self->shift->contractId)
                ->andReturn(Seq::from($self->examples->contracts[0]));
            $self->lookupContractUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::createShifts(), $self->examples->users[1]->id, $self->shift->contractId)
                ->andReturn(Seq::from($self->examples->contracts[3]));
            $self->lookupContractUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::createShifts(), $self->shift->userId, self::NOT_EXISTING_ID)
                ->andReturn(Seq::emptySeq());
            $self->lookupContractUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::createShifts(), self::NOT_EXISTING_ID, $self->shift->contractId)
                ->andReturn(Seq::emptySeq());

            $self->lookupStaffUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::createShifts(), $self->shift->assignerId)
            ...
            $self->lookupStaffUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::createShifts(), $self->examples->staffs[0]->id)
                ->andReturn(Seq::from($self->examples->staffs[0]));
            $self->lookupStaffUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::createShifts(), $self->examples->staffs[1]->id)
                ->andReturn(Seq::from($self->examples->staffs[1]));
            $self->lookupStaffUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::createShifts(), $self->examples->staffs[24]->id)
            ...
            $self->lookupUserUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::createShifts(), $self->shift->userId)
                ->andReturn(Seq::from($self->examples->users[0]));
            $self->lookupUserUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::createShifts(), $self->examples->users[1]->id)
    ...
    {
        $this->should('return Shift', function (): void {
    ...
    public function describe_validation(): void
    {
        ...
        $this->should('succeed when options is an array composed of multiple ServiceOption', function (): void {
            $validator = $this->request->createValidatorInstance(['options' => [ServiceOption::firstTime()->value(), ServiceOption::oneOff()->value()]] + $this->defaultInput());
            $this->assertFalse($validator->fails(), $validator->errors()->toJson(\JSON_UNESCAPED_UNICODE));
        });
    }
