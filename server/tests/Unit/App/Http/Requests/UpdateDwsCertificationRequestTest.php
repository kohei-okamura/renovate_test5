...
/**
 * {@link \App\Http\Requests\UpdateDwsCertificationRequest} のテスト
 */
final class UpdateDwsCertificationRequestTest extends Test
{
    use ConfigMixin;
    ...
    private UpdateDwsCertificationRequest $request;
    ...
    public static function _setUpSuite(): void
    {
        self::beforeEachSpec(function (self $self): void {
            ...
            $self->lookupOfficeUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), [Permission::updateDwsCertifications()], self::NOT_EXISTING_ID)
                ->andReturn(Seq::empty());

            $self->request = new UpdateDwsCertificationRequest();
    ...
    public function describe_payload(): void
    {
        $this->specify(
            ...
            function (): void {
                ...
                $this->assertEquals($expected, actual: $this->request->payload());
            }
        );
        $this->specify(
     ...
     * @return array
     */
    private function input(array $overwrites = []): array
    {
        return [
