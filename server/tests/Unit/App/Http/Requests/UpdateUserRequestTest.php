...
/**
 * UpdateUserRequest のテスト
 */
final class UpdateUserRequestTest extends Test
{
    use ConfigMixin;
    ...
    public static function _setUpSuite(): void
    {
        self::beforeEachSpec(function (self $self): void {
            $self->contractFinder
                ...
                ->byDefault();
            $self->request = new UpdateUserRequest();
    ...
    public function describe_validation(): void
    {
        ...
        $examples = [
            ...
            'when isEnabled is empty' => [
                ['isEnabled' => ['入力してください。']],
                ['isEnabled' => ''],
                ['isEnabled' => true],
            ],
            'when isEnabled is string' => [
        ...
        $this->should('fails when user status can update to false rules', function (): void {
            $input = $this->defaultInput();
            $this->contractFinder
                ->expects('find')
                ->withArgs(function (array $filter, array $pagination) use ($input): bool {
                    $expectFilter = [
                        'userId' => $input['id'],
                        'status' => [ContractStatus::formal(), ContractStatus::provisional()],
                    ];
                    $expectPagination = ['all' => true, 'sortBy' => 'id'];
                    $this->assertEquals($expectFilter, $filter);
                    $this->assertEquals($expectPagination, $pagination);
                    return true;
                })
                ->andReturn(FinderResult::from([$this->examples->contracts[0]], Pagination::create([])));
            $validator = $this->request->createValidatorInstance(['isEnabled' => false] + $input);
            $this->assertTrue($validator->fails());
            $this->assertEquals(
    ...
    private function expectedPayload(array $input): array
    {
        return [
            ...
            'birthday' => Carbon::create($input['birthday']),
            'addr' => new Addr(
