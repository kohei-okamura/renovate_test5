...
/**
 * UpdateAttendanceRequest のテスト
 */
class UpdateAttendanceRequestTest extends Test
{
    use CarbonMixin;
    use ConfigMixin;
    ...
    protected UpdateAttendanceRequest $request;
    ...
    {
        static::beforeEachSpec(function (UpdateAttendanceRequestTest $self): void {
            $self->request = new UpdateAttendanceRequest();
            ...
            $self->lookupStaffUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::updateAttendances(), $self->attendance->assignerId)
            ...
            $self->lookupStaffUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::updateAttendances(), $self->examples->staffs[0]->id)
                ->andReturn(Seq::from($self->examples->staffs[0]));
            $self->lookupStaffUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::updateAttendances(), $self->examples->staffs[1]->id)
                ->andReturn(Seq::from($self->examples->staffs[0]));
            $self->lookupStaffUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::updateAttendances(), $self->examples->staffs[24]->id)
                ->andReturn(Seq::from($self->examples->staffs[24]));

            $self->lookupOfficeUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), [Permission::updateAttendances()], $self->attendance->officeId)
            ...
            $self->lookupUserUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::updateAttendances(), $self->attendance->userId)
                ->andReturn(Seq::from($self->examples->users[0]));
            $self->lookupUserUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), Permission::updateAttendances(), $self->examples->users[1]->id)
    ...
    {
        $this->should('return array of Attendance', function (): void {
            ...
            $expected = [
                ...
                'assignees' => array_map(fn (array $assignee, int $index) => Assignee::create([
                    'sort_order' => $index,
                ...
                'durations' => array_map(fn (array $duration) => Duration::create([
                    'activity' => Activity::from($duration['activity']),
                    'duration' => $duration['duration'],
                ]), $input['durations']),
                'options' => array_map(
                    fn (int $option) => ServiceOption::from($option),
                    $input['options']
                ),
                'note' => $input['note'],
