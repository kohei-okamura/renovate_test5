 ...
 */
class CreateVisitingCareForPwsdCalcSpecRequestTest extends Test
{
    use CarbonMixin;
    use ConfigMixin;
    ...
    protected CreateVisitingCareForPwsdCalcSpecRequest $request;
    ...
    {
        static::beforeEachSpec(function (CreateVisitingCareForPwsdCalcSpecRequestTest $self): void {
            $self->visitingCareForPwsdCalcSpec = $self->examples->visitingCareForPwsdCalcSpecs[0];

            $self->request = new CreateVisitingCareForPwsdCalcSpecRequest();
     ...
     */
    private function expectedPayload(): VisitingCareForPwsdCalcSpec
    {
        $input = $this->defaultInput();
        $values = [
