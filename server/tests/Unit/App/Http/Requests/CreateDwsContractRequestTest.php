...
/**
 * {@link \App\Http\Requests\CreateDwsContractRequest} のテスト.
 */
final class CreateDwsContractRequestTest extends Test
{
    use CarbonMixin;
    use ConfigMixin;
    ...
    private CreateDwsContractRequest $request;
    ...
    public static function _setUpSuite(): void
    {
        ...
        self::beforeEachSpec(function (self $self): void {
            ...
            $self->lookupOfficeUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), [Permission::createDwsContracts()], $self->examples->offices[0]->id)
                ->andReturn(Seq::from($self->examples->offices[0]));
            $self->lookupOfficeUseCase
                ->allows('handle')
                ->with(anInstanceOf(Context::class), [Permission::createDwsContracts()], self::NOT_EXISTING_ID)
                ->andReturn(Seq::emptySeq());

            $self->request = new CreateDwsContractRequest();
