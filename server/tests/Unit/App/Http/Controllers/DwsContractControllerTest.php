...
/**
 * {@link \App\Http\Controllers\DwsContractController} のテスト.
 */
final class DwsContractControllerTest extends Test
{
    use CarbonMixin;
    use ContextMixin;
    ...
    private DwsContractController $controller;
    ...
    public static function _setUpSuite(): void
    {
        self::beforeEachSpec(function (self $self): void {
            $self->contract = $self->examples
                ->contracts[0]
                ->copy(['organizationId' => $self->examples->organizations[0]->id]);
            ...
            $self->controller = app(DwsContractController::class);
        });
    }
    ...
    public function describe_create(): void
    {
        ...
        app()->bind(CreateDwsContractRequest::class, function () {
            $request = Mockery::mock(CreateDwsContractRequest::class)->makePartial();
    ...
    public function describe_get(): void
    {
        ...
        $this->should('get contract using use case', function (): void {
            $this->lookupContractUseCase
                ->expects('handle')
                ->with($this->context, Permission::viewDwsContracts(), $this->contract->userId, $this->contract->id)
        ...
        $this->should('throw a NotFoundException when the id not exists in db', function (): void {
            $this->lookupContractUseCase
                ->expects('handle')
                ->with($this->context, Permission::viewDwsContracts(), $this->contract->userId, self::NOT_EXISTING_ID)
    ...
    public function describe_update(): void
    {
        ...
        app()->bind(UpdateDwsContractRequest::class, function () {
            $request = Mockery::mock(UpdateDwsContractRequest::class)->makePartial();
            $request->allows('context')->andReturn($this->context)->byDefault();
