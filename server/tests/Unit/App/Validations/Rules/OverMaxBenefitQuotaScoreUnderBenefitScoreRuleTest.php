...
/**
 * {@link \App\Validations\Rules\OverMaxBenefitQuotaScoreUnderBenefitScoreRule} のテスト.
 */
final class OverMaxBenefitQuotaScoreUnderBenefitScoreRuleTest extends Test
{
    ...
    {
        $this->should(
            'pass when plan maxBenefitQuotaExcessScore is under benefitScore',
            function (): void {
                $validator = $this->buildCustomValidator(
                    ...
                    ['plan.maxBenefitQuotaExcessScore' => 'over_max_benefit_quota_score_under_benefit_score']
                );
                $this->assertTrue($validator->passes());
            }
        );
        $this->should(
            'pass when result maxBenefitQuotaExcessScore is under benefitScore',
            function (): void {
                $validator = $this->buildCustomValidator(
                    [
                        ...$this->createParams([
                    ...
                    ['result.maxBenefitQuotaExcessScore' => 'over_max_benefit_quota_score_under_benefit_score']
                );
                $this->assertTrue($validator->passes());
            }
        );
        $this->should(
            'fail when plan OverMaxBenefitScore is not under benefitScore',
            function (): void {
                $validator = $this->buildCustomValidator(
                    [
                        ...$this->createParams([
                            'plan' => [
                                'maxBenefitExcessScore' => 8000,
                                'maxBenefitQuotaExcessScore' => 3000,
                            ],
                        ]),
                    ],
                    ['plan.maxBenefitQuotaExcessScore' => 'over_max_benefit_quota_score_under_benefit_score']
        ...
        $this->should(
            'fail when result OverMaxBenefitScore is not under benefitScore',
            function (): void {
                $validator = $this->buildCustomValidator(
                    [
                        ...$this->createParams([
