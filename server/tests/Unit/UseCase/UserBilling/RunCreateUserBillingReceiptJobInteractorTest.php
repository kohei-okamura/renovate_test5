...
use Tests\Unit\Mixins\GenerateUserBillingReceiptPdfUseCaseMixin;
...
/**
 * RunCreateUserBillingReceiptJobInteractor のテスト.
 */
final class RunCreateUserBillingReceiptJobInteractorTest extends Test
{
    use ConfigMixin;
    use ContextMixin;
    use ExamplesConsumer;
    use GenerateUserBillingReceiptPdfUseCaseMixin;
    use MockeryMixin;
    ...
    private RunCreateUserBillingReceiptJobInteractor $interactor;
    ...
    {
        static::beforeEachSpec(function (RunCreateUserBillingReceiptJobInteractorTest $self): void {
            $self->ids = [
            ...
            $self->runJobUseCase
                ...
                ->andReturnNull()
                ->byDefault();
            $self->generateUserBillingReceiptPdfUseCase
                ->allows('handle')
                ->andReturn(self::PATH)
                ->byDefault();
            $self->config
            ...
            $self->interactor = app(RunCreateUserBillingReceiptJobInteractor::class);
        });
    }
    ...
    public function describe_handle(): void
    {
        ...
        $this->should('call GenerateUserBillingReceiptPdfUseCase', function (): void {
            ...
            $this->runJobUseCase
                ...
                ->andReturnUsing(
                    function (Context $context, DomainJob $job, Closure $f) use ($uri): void {
                        $this->generateUserBillingReceiptPdfUseCase
                            ->expects('handle')
