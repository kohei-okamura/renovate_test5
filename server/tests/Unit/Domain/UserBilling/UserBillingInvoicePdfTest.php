...
/**
 * {@link \Domain\UserBilling\UserBillingInvoicePdf} のテスト.
 */
final class UserBillingInvoicePdfTest extends Test
{
    ...
    public function describe_from(): void
    {
        $this->should('return an instance', function (): void {
            ...
            $expected = UserBillingInvoicePdf::create([
            ...
            $actual = UserBillingInvoicePdf::from($user, $billing, $issuedOn);
     ...
     *
     * @param array $attrs
     * @return \Domain\UserBilling\UserBillingInvoicePdf
     */
    private function createInstance(array $attrs = []): UserBillingInvoicePdf
    {
        $user = $this->examples->users[0];
        $billing = $this->examples->userBillings[0];
        $amounts = self::calculateAmounts($billing);
        ...
        return UserBillingInvoicePdf::create($attrs + $values);
    }
