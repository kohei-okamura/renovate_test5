...
/**
 * {@link \Infrastructure\ServiceCodeDictionary\LtcsBillingRepositoryEloquentImpl} のテスト.
 */
final class LtcsBillingRepositoryEloquentImplTest extends Test
{
    use CarbonMixin;
    use DatabaseMixin;
    ...
    private LtcsBillingRepositoryEloquentImpl $repository;
    ...
    {
        self::beforeEachSpec(function (LtcsBillingRepositoryEloquentImplTest $self): void {
            $self->repository = app(LtcsBillingRepositoryEloquentImpl::class);
        });
    }
    ...
    public function describe_lookup(): void
    {
        $this->should('return an entity when the id exists in db', function (): void {
            $expected = $this->examples->ltcsBillings[0];
    ...
    public function describe_store(): void
    {
        $this->should('add the entity to repository when it does not exist in repository', function (): void {
            $entity = $this->examples->ltcsBillings[0]->copy(['id' => 20200925]);
        ...
        $this->should('update the entity', function (): void {
            $entity = $this->examples->ltcsBillings[0]->copy(['name' => 'test_name']);
    ...
    public function describe_remove(): void
    {
        $this->should('remove the entity', function (): void {
            $entity = $this->examples->ltcsBillings[0];
    ...
    public function describe_removeById(): void
    {
        ...
        $this->should('not remove other entities', function (): void {
            $entities = [
                $this->examples->ltcsBillings[0],
                $this->examples->ltcsBillings[1],
                $this->examples->ltcsBillings[2],
                $this->examples->ltcsBillings[3],
            ];
