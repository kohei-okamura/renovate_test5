...
/**
 * {@link \Infrastructure\ServiceCodeDictionary\LtcsBillingInvoiceRepositoryEloquentImpl} のテスト.
 */
final class LtcsBillingInvoiceRepositoryEloquentImplTest extends Test
{
    use CarbonMixin;
    use DatabaseMixin;
    ...
    private LtcsBillingInvoiceRepositoryEloquentImpl $repository;
    ...
    {
        self::beforeEachSpec(function (LtcsBillingInvoiceRepositoryEloquentImplTest $self): void {
            $self->repository = app(LtcsBillingInvoiceRepositoryEloquentImpl::class);
        });
    }
    ...
    public function describe_lookup(): void
    {
        $this->should('return an entity when the id exists in db', function (): void {
            $expected = $this->examples->ltcsBillingInvoices[0];
    ...
    public function describe_store(): void
    {
        $this->should('add the entity to repository when it does not exist in repository', function (): void {
            $entity = $this->examples->ltcsBillingInvoices[0]->copy(['id' => 20200925]);
        ...
        $this->should('update the entity', function (): void {
            $entity = $this->examples->ltcsBillingInvoices[0]->copy([
                'copayAmount' => 9999,
            ]);
    ...
    public function describe_remove(): void
    {
        $this->should('remove the entity', function (): void {
            $entity = $this->examples->ltcsBillingInvoices[0];
    ...
    public function describe_removeById(): void
    {
        $this->should('remove entities', function (): void {
            $entities = [
                ...
                $this->examples->ltcsBillingInvoices[1],
            ];
        ...
        $this->should('not remove other entities', function (): void {
            $entities = [
                $this->examples->ltcsBillingInvoices[0],
                $this->examples->ltcsBillingInvoices[1],
                $this->examples->ltcsBillingInvoices[2],
                $this->examples->ltcsBillingInvoices[3],
            ];
    ...
    public function describe_lookupByBundleId(): void
    {
        $this->should('return Map of Seq with Bundle ID of key', function () {
            ...
            $actual->each(function (Seq $x, int $key) use ($ids): void {
                ...
                $this->assertForAll($x, fn (LtcsBillingInvoice $invoice): bool => $invoice->bundleId === $key);
            });
        });
    }
}
