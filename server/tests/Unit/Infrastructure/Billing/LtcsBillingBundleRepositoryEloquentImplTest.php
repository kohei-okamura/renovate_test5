...
/**
 * {@link \Infrastructure\ServiceCodeDictionary\LtcsBillingBundleRepositoryEloquentImpl} のテスト.
 */
final class LtcsBillingBundleRepositoryEloquentImplTest extends Test
{
    use CarbonMixin;
    use DatabaseMixin;
    ...
    private LtcsBillingBundleRepositoryEloquentImpl $repository;
    ...
    {
        self::beforeEachSpec(function (LtcsBillingBundleRepositoryEloquentImplTest $self): void {
            $self->repository = app(LtcsBillingBundleRepositoryEloquentImpl::class);
        });
    }
    ...
    public function describe_lookup(): void
    {
        $this->should('return an entity when the id exists in db', function (): void {
            $expected = $this->examples->ltcsBillingBundles[0];
    ...
    public function describe_store(): void
    {
        $this->should('add the entity to repository when it does not exist in repository', function (): void {
            $entity = $this->examples->ltcsBillingBundles[0]->copy(['id' => 20200925]);
        ...
        $this->should('update the entity', function (): void {
            $entity = $this->examples->ltcsBillingBundles[0]->copy(['name' => 'test_name']);
    ...
    public function describe_remove(): void
    {
        $this->should('remove the entity', function (): void {
            $entity = $this->examples->ltcsBillingBundles[0];
    ...
    public function describe_removeById(): void
    {
        $this->should('remove entities', function (): void {
            $entities = [
                ...
                $this->examples->ltcsBillingBundles[1],
            ];
        ...
        $this->should('not remove other entities', function (): void {
            $entities = [
                $this->examples->ltcsBillingBundles[0],
                $this->examples->ltcsBillingBundles[1],
                $this->examples->ltcsBillingBundles[2],
                $this->examples->ltcsBillingBundles[3],
            ];
    ...
    public function describe_lookupByBillingId(): void
    {
        $this->should('return Map of Seq with Billing ID of key', function () {
            ...
            $actual->each(function (Seq $x, int $key) use ($ids): void {
                ...
                $this->assertForAll($x, fn (LtcsBillingBundle $bundle): bool => $bundle->billingId === $key);
            });
        });
    }
}
