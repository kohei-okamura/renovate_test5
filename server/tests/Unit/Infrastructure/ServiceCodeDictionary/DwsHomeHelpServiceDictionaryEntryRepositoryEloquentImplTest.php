...
/**
 * {@link Infrastructure\ServiceCodeDictionary\DwsHomeHelpServiceDictionaryEntry} Test
 */
class DwsHomeHelpServiceDictionaryEntryRepositoryEloquentImplTest extends Test
{
    use DatabaseMixin;
    use ExamplesConsumer;
    use UnitSupport;

    private DwsHomeHelpServiceDictionaryEntryRepositoryEloquentImpl $repository;
    ...
    {
        static::beforeEachSpec(function (DwsHomeHelpServiceDictionaryEntryRepositoryEloquentImplTest $self): void {
            $self->repository = app(DwsHomeHelpServiceDictionaryEntryRepositoryEloquentImpl::class);
        });
    }
    ...
    public function describe_lookup(): void
    {
        $this->should('return an entity when the id exists in db', function (): void {
            $entry = $this->examples->dwsHomeHelpServiceDictionaryEntries[0];
            $expected = $entry;
    ...
    public function describe_store(): void
    {
        $this->should('return the entity with id when an entity of not having id stored', function (): void {
            $entity = $this->examples->dwsHomeHelpServiceDictionaryEntries[1]->copy(['id' => null]);
        ...
        $this->should('update the dictionary', function (): void {
            $entry = $this->examples->dwsHomeHelpServiceDictionaryEntries[1];
    ...
    public function describe_removeById(): void
    {
        $this->should('remove entities', function (): void {
            $entry0 = $this->examples->dwsHomeHelpServiceDictionaryEntries[0];
            $entry1 = $this->examples->dwsHomeHelpServiceDictionaryEntries[1];
