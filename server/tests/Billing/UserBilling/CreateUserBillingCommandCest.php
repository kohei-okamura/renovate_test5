 ...
 * 利用者請求生成コマンドのテスト.
 */
final class CreateUserBillingCommandCest extends Test
{
     ...
     * * Artisan Command テスト.
     *
     * - すべてのサービスが存在する場合の利用者請求生成.
     * - ただしその他サービスはまだ未対応のため除く.
     * - データをここで登録しているので単体で動かすとスナップショットのIDがずれる。手間が半端ないので後で直す。
     *
    ...
    public function succeedArtisanCommand(BillingTester $I): void
    {
        ...
        $ltcsProvisionReportRepository->store($this->examples->ltcsProvisionReports[0]->copy([
            'id' => null,
            'userId' => $user->id,
            'officeId' => $office->id,
            'providedIn' => Carbon::now()->subMonth()->firstOfMonth(),
        ...
        $actual = $userBillings->map(
            function (UserBilling $x) use ($dwsBillingStatement, $ltcsBillingStatement, $office, $user, $I) {
