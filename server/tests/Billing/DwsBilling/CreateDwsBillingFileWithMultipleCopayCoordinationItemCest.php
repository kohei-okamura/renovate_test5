...
/**
 * Billing files Create (居宅) のテスト. 予実から生成し、上限管理対象が9件の場合.
 */
final class CreateDwsBillingFileWithMultipleCopayCoordinationItemCest extends CreateDwsBillingTest
{
    ...
    public function succeedAPICall(BillingTester $I): void
    {
        ...
        //
        // 予実を準備
        $this->storeHomeHelpServiceProvisionReport($office);
        $this->storeVisitingCareForPwsdProvisionReport($office);

        //
        // 請求生成処理
        $I->sendPOST('dws-billings', ['officeId' => $office->id, 'transactedIn' => '2021-06']);
        ...
        $bundles = $this->getBundleRepository()->lookupByBillingId($billingId)->values()->flatten();

        /** @var int[] $bundleIds */
        $bundleIds = $bundles->map(fn (DwsBillingBundle $x): int => $x->id);

        /** @var \Domain\Billing\DwsBillingStatement[]|\ScalikePHP\Seq $bundles */
        $statements = $this->getStatementRepository()
            ...
            $I->sendPost(
                ...
                [
                    ...
                    'items' => [
                        ...
                        [
                            'officeId' => $this->examples->offices[8]->id,
                            'subtotal' => [
                                'fee' => 100000,
                                'copay' => 9300,
                                'coordinatedCopay' => 0,
                        ...
                        [
                            'officeId' => $this->examples->offices[10]->id,
                            'subtotal' => [
                                'fee' => 100000,
                                'copay' => 9300,
                                'coordinatedCopay' => 0,
                        ...
                        [
                            'officeId' => $this->examples->offices[12]->id,
                            'subtotal' => [
                                'fee' => 100000,
                                'copay' => 9300,
                                'coordinatedCopay' => 0,
        ...
        $I->assertCount(12, $latest['billing']['files'], 'Number of files');
     ...
     * @return void
     */
    private function storeHomeHelpServiceProvisionReport(Office $office): void
    {
        ...
        $reportResults = [
            $baseItem->copy([
                'schedule' => Schedule::create([
                    'date' => Carbon::create(2021, 5, 3),
                    'start' => Carbon::create(2021, 5, 3, 10, 0),
                    'end' => Carbon::create(2021, 5, 3, 11, 30),
                ]),
            ]),
        ];
     ...
     * @return void
     */
    private function storeVisitingCareForPwsdProvisionReport(Office $office): void
    {
