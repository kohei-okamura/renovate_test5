 ...
 * 障害福祉サービス：予実詳細：合計.
 *
 * {@link \Domain\Billing\DwsBillingServiceReportAggregateGroup} をキー, 小数値を値とするマップの実装.
 */
final class DwsProvisionReportTimeSummaryItem implements JsonSerializable, Equatable
{
    ...
    public static function fromAssoc(array $assoc): self
    {
        $map = Map::from($assoc);
        $map->keys()
            ->find(fn (int $category): bool => !DwsBillingServiceReportAggregateGroup::isValid($category))
     ...
     *
     * @param \Domain\Billing\DwsBillingServiceReportAggregateGroup $category
     * @return \Domain\Common\Decimal[]&\ScalikePHP\Option
     */
    public function getOption(DwsBillingServiceReportAggregateGroup $category): Option
    {
