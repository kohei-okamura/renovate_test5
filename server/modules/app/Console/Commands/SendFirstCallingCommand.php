 ...
 * 出勤確認第一通知コマンド.
 */
class SendFirstCallingCommand extends Command
{
     ...
     * @var string
     */
    protected $description = '出勤確認第一通知';
    ...
    public function handle(OrganizationIterator $iterator, QueueingDispatcher $dispatcher)
    {
        try {
            ...
            $iterator->iterate(function (Context $context) use ($dispatcher, $range): void {
                $dispatcher->dispatch(new SendFirstCallingJob($context, $range));
            });
    ...
    private function getRange(): CarbonRange
    {
        ...
        $start = $target->addMinutes(Calling::FIRST_TARGET_MINUTES);
