 ...
 * 出勤確認第二通知コマンド.
 */
final class SendSecondCallingCommand extends Command
{
     ...
     * @var string
     */
    protected $description = '出勤確認第二通知';
     ...
     * @return int
     */
    public function handle(OrganizationIterator $iterator, QueueingDispatcher $dispatcher)
    {
        try {
            $range = $this->getRange();
            $iterator->iterate(function (Context $context) use ($dispatcher, $range): void {
                $dispatcher->dispatch(new SendSecondCallingJob($context, $range));
            });
    ...
    private function getRange(): CarbonRange
    {
        ...
        $start = $target->addMinutes(Calling::SECOND_TARGET_MINUTES);
