 ...
 * 出勤確認第三通知コマンド.
 */
class SendThirdCallingCommand extends Command
{
     ...
     * @var string
     */
    protected $description = '出勤確認第三通知';
    ...
    public function handle(OrganizationIterator $iterator, QueueingDispatcher $dispatcher): int
    {
        try {
            ...
            $iterator->iterate(function (Context $context) use ($dispatcher, $range): void {
                $dispatcher->dispatch(new SendThirdCallingJob($context, $range));
            });
    ...
    private function getRange(): CarbonRange
    {
        ...
        $start = $target->addMinutes(Calling::THIRD_TARGET_MINUTES);
