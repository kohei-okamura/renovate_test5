 ...
 */
class CreateDwsCertificationRequest extends StaffRequest implements ValidatesWhenResolved
{
    use FormRequest;
     ...
     */
    public function payload(): DwsCertification
    {
        ...
        $agreements = Seq::fromArray($this->agreements)
            ...
            ->toArray();
        return DwsCertification::create([
            ...
            'copayRate' => 10, // 利用者負担割合を設定可能にしていたが、本来は1割だったため固定で1割に設定する形で対応.
            ...
            'copayCoordination' => CopayCoordination::create([
                ...
                'officeId' => empty($this->copayCoordination['officeId']) ? null : $this->copayCoordination['officeId'],
            ]),
    ...
    protected function rules(array $input): array
    {
        return [
            ...
            'dwsTypes.*' => ['required', 'integer', 'dws_type'],
            ...
            'agreements.*.indexNumber' => ['required', 'integer'],
            'agreements.*.officeId' => ['required', 'office_exists:' . Permission::createDwsCertifications()],
