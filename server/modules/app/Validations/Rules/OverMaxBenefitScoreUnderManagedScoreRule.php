...
/**
 * 種類支給限度基準を超える単位数が限度額管理対象単位数以下より低い値であることを検証する.
 *
 * @mixin \App\Validations\CustomValidator
 */
trait OverMaxBenefitScoreUnderManagedScoreRule
{
    /**
     * 検証処理.
     *
     ...
     */
    protected function validateOverMaxBenefitScoreUnderManagedScore(string $attribute, mixed $value, array $parameters): bool
    {
        $maxBenefitExcessScore = $value;
        // 種類支給限度基準を超える単位数が0より小さいまたは、数値でない場合は別のバリデーションで検証する
        if ($maxBenefitExcessScore < 0 || !is_numeric($maxBenefitExcessScore)) {
            return true;
        }
        ...
        [$planOrResult => $scoreSummary] = $useCase->handle(
            ...
            // TODO: 一時的に0にしておく。本当はリクエストの値をつかす
        ...
        return $maxBenefitExcessScore <= $scoreSummary['managedScore'];
    }
}
