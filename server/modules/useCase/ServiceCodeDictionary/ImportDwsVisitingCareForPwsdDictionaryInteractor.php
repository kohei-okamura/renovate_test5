...
/**
 * 障害福祉サービス：重度訪問介護：サービスコード辞書インポート実装.
 */
final class ImportDwsVisitingCareForPwsdDictionaryInteractor implements ImportDwsVisitingCareForPwsdDictionaryUseCase
{
    use Logging;

    private DwsVisitingCareForPwsdDictionaryRepository $repository;
    private DwsVisitingCareForPwsdDictionaryEntryRepository $entryRepository;
     ...
     * Constructor.
     *
     * @param \Domain\ServiceCodeDictionary\DwsVisitingCareForPwsdDictionaryRepository $repository
     * @param \Domain\ServiceCodeDictionary\DwsVisitingCareForPwsdDictionaryEntryRepository $entryRepository
     * @param \Domain\File\ReadonlyFileStorage $storage
     * @param \Domain\TransactionManagerFactory $factory
     */
    public function __construct(
        DwsVisitingCareForPwsdDictionaryRepository $repository,
        DwsVisitingCareForPwsdDictionaryEntryRepository $entryRepository,
    ...
    public function handle(int $id, string $filepath, string $effectivatedOn, string $name): int
    {
        ...
        $csv = DwsVisitingCareForPwsdDictionaryCsv::create(Csv::read($file->getPathname()));

        return $this->transaction->run(function () use ($id, $effectivatedOn, $name, $csv): int {
            $version = $this->repository->lookup($id)
                ->headOption()
                ->map(fn ($x) => $x->version + 1)
                ->getOrElseValue(1);
            $this->repository->store(
                DwsVisitingCareForPwsdDictionary::create([
                    'id' => $id,
            ...
            $rows = $csv->rows();
            $attrs = ['dwsVisitingCareForPwsdDictionaryId' => $id];
            foreach ($rows as $row) {
