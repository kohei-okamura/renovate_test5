...
<template>
  <z-data-accordion
    class="z-visiting-care-for-pwsd-calc-specs-card"
  ...
  >
    ...
    <template #item.specifiedOfficeAddition="{ item }">
      {{ resolveVisitingCareForPwsdSpecifiedOfficeAddition(item.specifiedOfficeAddition) }}
    </template>
    ...
    <template #footer>
      <v-btn v-if="isExpandable" block color="white" data-toggle-expanded-btn @click="toggleExpanded">
        {{ isExpanded ? '古い算定情報（障害・重度訪問介護）を隠す' : '古い算定情報（障害・重度訪問介護）を表示' }}
...
import { resolveVisitingCareForPwsdSpecifiedOfficeAddition } from '@zinger/enums/lib/visiting-care-for-pwsd-specified-office-addition'
...
type Props = {
  items: VisitingCareForPwsdCalcSpec[]
  office: Office
}
...
export default defineComponent<Props>({
  name: 'ZVisitingCareForPwsdCalcSpecsCard',
  props: {
  ...
  setup: props => {
    ...
    const accordionOptions = computed<ZDataAccordionOptions<VisitingCareForPwsdCalcSpec>>(() => ({
      content: '算定情報（障害・重度訪問介護）',
      headers: [
      ...
      itemLink: x => `/offices/${props.office.id}/visiting-care-for-pwsd-calc-specs/${x.id}/edit`,
      itemLinkPermissions: [permissions.updateInternalOffices, permissions.updateExternalOffices],
      itemLinkText: '編集',
      title: '算定情報（障害・重度訪問介護）'
    }))
    ...
    return {
      ...
      resolveVisitingCareForPwsdSpecifiedOfficeAddition,
      toggleExpanded
    }
  }
})
