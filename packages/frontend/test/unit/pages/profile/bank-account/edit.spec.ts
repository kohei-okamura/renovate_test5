...
import BankAccountEditPage from '~/pages/profile/bank-account/edit.vue'
...
describe('pages/profile/bank-account/edit.vue', () => {
  ...
  function mountComponent () {
    wrapper = mount(BankAccountEditPage, {
      ...provides(
  ...
  describe('submit', () => {
    ...
    afterAll(() => {
      ...
      mocked(staffStore.updateBankAccount).mockRestore()
    })

    afterEach(() => {
      mocked($snackbar.success).mockClear()
      mocked($snackbar.error).mockClear()
      mocked(staffStore.updateBankAccount).mockClear()
    })
    ...
    it('should display message when succeeded', async () => {
      ...
      expect($snackbar.success).toHaveBeenCalledWith('給与振込口座を登録しました。')
    })
