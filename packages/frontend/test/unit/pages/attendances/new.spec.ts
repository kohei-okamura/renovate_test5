...
import AttendanceNewPage from '~/pages/attendances/new.vue'
...
describe('pages/attendances/new.vue', () => {
  ...
  const form: Partial<AttendancesApi.Form> = {
    officeId,
    userId: USER_ID_MIN,
  ...
  function mountComponent (options: Partial<MountOptions<Vue>> = {}) {
    wrapper = mount(AttendanceNewPage, {
      ...options,
      mocks
    })
  }
  ...
  describe('submit', () => {
    ...
    beforeEach(() => {
      jest.spyOn($api.attendances, 'create').mockResolvedValue(undefined)
    ...
    afterEach(() => {
      ...
      mocked($api.attendances.create).mockReset()
    })

    it('should call $api.attendances.create', async () => {
      await wrapper.vm.submit(form)

      expect($api.attendances.create).toHaveBeenCalledTimes(1)
      expect($api.attendances.create).toHaveBeenCalledWith({ form })
    })
    ...
    it('should display message', async () => {
      ...
      expect($snackbar.success).toHaveBeenCalledWith('勤務実績情報を登録しました。')
    })
    ...
    it.each([
      ...
      async (key, message, testId = undefined) => {
        jest.spyOn($api.attendances, 'create').mockRejectedValue(createAxiosError(HttpStatusCode.BadRequest, {
          errors: {
            [key]: [message]
          }
        }))
