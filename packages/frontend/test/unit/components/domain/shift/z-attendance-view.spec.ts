...
describe('z-attendance-view.vue', () => {
  const { mount } = setupComponentTest()
  const propsData = createAttendanceResponseStub()
  let wrapper: Wrapper<Vue>

  function mountComponent (options: MountOptions<Vue>, auth: Partial<Auth> = { isSystemAdmin: true }) {
    wrapper = mount(ZAttendanceView, {
      ...options,
      ...provides(
  ...
  describe.each([
    ['user', '[data-user-card-item]', Permission.viewUsers],
    ['staff', '[data-staff-card-item]', Permission.viewStaffs]
  ...
  describe('display status', () => {
    const baseAttendance = propsData.attendance

    it('should be displayed "キャンセル" if attendance has been canceled', () => {
      const status = { isCanceled: true }
      const propsData = { attendance: { ...baseAttendance, ...status } }
      mountComponent({ propsData })
      expect(wrapper.find('[data-status]').text()).toContain('キャンセル')
      unmountComponent()
    })

    it('should be displayed "確定" if attendance has been confirmed and not canceled', () => {
    ...
    it('should be displayed "未確定" if attendance has neither confirmed nor canceled', () => {
  ...
  describe('display cancel reason', () => {
    ...
    it('should not be displayed cancel reason if attendance has not been canceled', () => {
      const status = { isCanceled: false }
      const propsData = { attendance: { ...baseAttendance, ...status } }
      mountComponent({ propsData })
      expect(wrapper.find('[data-cancel-reason]')).not.toExist()
      unmountComponent()
    })

    it('should be displayed cancel reason if attendance has been canceled', () => {
      const status = { isCanceled: true }
      const propsData = { attendance: { ...baseAttendance, ...status } }
      mountComponent({ propsData })
      expect(wrapper.find('[data-cancel-reason]')).toExist()
      unmountComponent()
    })
  })
})
