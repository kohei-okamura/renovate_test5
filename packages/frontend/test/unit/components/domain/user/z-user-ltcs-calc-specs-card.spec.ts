...
describe('z-user-ltcs-calc-specs-card.vue', () => {
  const { mount } = setupComponentTest()
  const user = createUserStub()
  let wrapper: Wrapper<Vue & any>

  beforeAll(() => {
    const items = createUserLtcsCalcSpecStubsForUser(user.id)
    const propsData = { items, user }
    wrapper = mount(ZUserLtcsCalcSpecsCard, {
      ...provides(
  ...
  it('should be create itemLink', () => {
    ...
    const expectedLink = `/users/${user.id}/ltcs-calc-specs/${item.id}/edit`
