...
describe('z-bank-account-form.vue', () => {
  ...
  function mountComponent (options: MountOptions<Vue> = {}) {
    wrapper = mount(ZBankAccountForm, {
      mocks,
      propsData,
      ...options
    })
  ...
  describe('validation', () => {
    ...
    beforeAll(async () => {
      ...
      observer = getValidationObserver(wrapper)
      /*
       * ToDo ここで一度 validate を呼んでおかないと 'should pass when input correctly' が失敗する
       * @vue/composition-api: "1.0.0-beta.11" まではこれがなくても通っていた
       */
      await observer.validate()
    })
